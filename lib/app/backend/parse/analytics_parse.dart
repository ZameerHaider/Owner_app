...
class AnalyticsParser {
  ...
  Future<Response> getStats(var month, var year) async {
    return await apiService.postPrivate(
        AppConstants.getStats, {'id': getUID(), "month": month, "year": year, "type": getType()}, sharedPreferencesManager.getString('token') ?? '');
  }
  ...
  Future<Response> getStatsProducts(var month, var year) async {
    return await apiService.postPrivate(
        AppConstants.getStatsProducts, {'id': getUID(), "month": month, "year": year, "type": getType()}, sharedPreferencesManager.getString('token') ?? '');
  }

...
class AppointmentParser {
  ...
  Future<Response> getSalonList() async {
    var response =
        await apiService.postPrivate(AppConstants.getSalonAppointmentsList, {"id": sharedPreferencesManager.getString('uid')}, sharedPreferencesManager.getString('token') ?? '');
    return response;
  }

  Future<Response> getIndividualAppointmentsList() async {
    var response = await apiService.postPrivate(
        AppConstants.getIndividualAppointmentsList, {"id": sharedPreferencesManager.getString('uid')}, sharedPreferencesManager.getString('token') ?? '');
    return response;
  }
